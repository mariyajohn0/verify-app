...
function PhoneVerification() {
  ...
  // Fetching phone number from Redux store
  ...
  const sendPhoneOtp = async () => {
    ...
    const reqBody = { phoneNumber: phone }; // Request body for API call
    try {
  ...
  const verifyOtp = async () => {
    ...
    const reqBody = { phone, otp: otpString }; // Request body with phone number and OTP
    try {
  ...
  const handleChange = (index: number, value: string) => {
    ...
    if (value && index < otp.length - 1) {
      inputRefs.current[index + 1]?.focus(); // Move focus to the next input field if a value is entered
    }

    if (!value && index > 0) {
      inputRefs.current[index - 1]?.focus(); // Move focus to the previous input field if a value is deleted
    }
      ...
      <div className={styles.card}>
        ...
        <div className={styles.verification}>
          ...
          <div className={styles.buttons}>
            ...
            <br />
            <button onClick={resendOtp} disabled={isResending}>
        ...
        {isOtpVerified && (
          <p className={styles.verifiedMessage}>Phone number verified!</p>
        )}
      </div>
    </div>
  );
